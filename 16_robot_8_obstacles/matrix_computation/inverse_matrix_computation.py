...
alpha_ij = zeros((n_con+n_con_1)*num_horizon)
beta_ij = zeros((n_con+n_con_1)*num_horizon)
d_obs_ij = ones((n_con+n_con_1)*num_horizon)

lamda_wc = ones((n_con+n_con_1)*num_horizon)
lamda_ws = ones((n_con+n_con_1)*num_horizon)
...
Ax_eq_obs, Ay_eq_obs, Az_eq_obs = optim_inv_mat_computation.compute_obs_mat(P, num, n_rob, n_con, n_d)
