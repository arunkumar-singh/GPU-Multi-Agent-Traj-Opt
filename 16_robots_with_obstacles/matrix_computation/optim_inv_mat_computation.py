...
def compute_obs_mat(P, num, n_rob, n_con, n_d):
    ...
    block = vstack([P]*n_d)
    block_reshape = [block]*n_rob
    temp = block_diag(*block_reshape)

    Ay_eq_obs = Ax_eq_obs
    Az_eq_obs = Ax_eq_obs

    Ax_eq_obs = vstack(( Ax_eq_obs, temp  ))

    return Ax_eq_obs,  Ay_eq_obs, Az_eq_obs
